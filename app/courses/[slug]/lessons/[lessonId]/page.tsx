...
import { CheckCircle2, Circle, ClipboardCheck, ClipboardList, ExternalLink } from "lucide-react";
...
export default async function LessonPage({ params, searchParams }: { params: Promise<{ slug: string; lessonId: string }>; searchParams: Promise<{ notice?: string; error?: string }> }) {
  ...
  const [{ data: progress }, { data: quizzes }, { data: tasks }] = await Promise.all([
    ...
    supabase.from("tasks").select("*").eq("lesson_id", lessonId).eq("is_published", true).order("deadline", { ascending: true, nullsFirst: false }),
  ]);
