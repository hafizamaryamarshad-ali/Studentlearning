...
export default async function AdminStudentsPage() {
  ...
  return <DashboardShell admin eyebrow="Student management" title="Students" profile={profile}>
    ...
    {error ? <Card className="border-red-200 bg-red-50 p-6 text-red-900">Student records could not be loaded. Check the database connection and try again.</Card> : !students?.length ? <Card className="p-10 text-center"><Users className="mx-auto h-10 w-10 text-slate-400" /><h2 className="mt-4 text-xl font-extrabold">No students yet</h2><p className="mt-2 text-slate-600">Verified student accounts will appear here after signup.</p></Card> : <Card className="overflow-hidden p-0"><Table><TableHeader><TableRow><TableHead className="px-5">Student</TableHead><TableHead>Email</TableHead><TableHead>Enrollments</TableHead><TableHead className="pr-5">Joined</TableHead></TableRow></TableHeader><TableBody>{students.map((student) => <TableRow key={student.id}><TableCell className="px-5 font-extrabold">{student.full_name}</TableCell><TableCell>{student.email}</TableCell><TableCell>{enrollments.filter((enrollment) => enrollment.student_id === student.id).length}</TableCell><TableCell className="pr-5">{new Intl.DateTimeFormat("en", { dateStyle: "medium" }).format(new Date(student.created_at))}</TableCell></TableRow>)}</TableBody></Table></Card>}
  </DashboardShell>;
}
