...
export default async function EditCoursePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ notice?: string; error?: string }> }) {
  ...
  return <DashboardShell admin eyebrow="Course management" title={course.title} profile={profile}>
    ...
    <Card className="mt-6 p-6"><h2 className="text-xl font-extrabold">Student visibility</h2><p className="mt-2 text-sm leading-6 text-slate-600">Publish shows the course to students. Remove from students archives it while preserving enrollments, payments, and certificates.</p><div className="mt-5 flex flex-wrap gap-3">{(["draft", "published", "archived"] as const).map((status) => <form action={changeCourseStatus} key={status}><input type="hidden" name="id" value={course.id} /><input type="hidden" name="status" value={status} /><Button type="submit" variant={course.status === status ? "default" : "outline"} disabled={course.status === status}>{status === "archived" ? "Remove from students" : status === "published" ? "Publish" : "Keep as draft"}</Button></form>)}</div></Card>
