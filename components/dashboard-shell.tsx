...
export function DashboardShell({ title, eyebrow, children, profile, admin = false }: {
  ...
  const adminPlaceholders = [
    [Users, "Students"], [Trophy, "Challenges"],
      ...
      <div className="mx-auto grid max-w-[1440px] md:grid-cols-[250px_1fr]">
        <aside className="hidden min-h-[calc(100vh-68px)] border-r border-slate-200 bg-white p-5 md:flex md:flex-col">
          ...
          <nav className="mt-4 flex-1 space-y-1" aria-label={admin ? "Admin navigation" : "Student navigation"}>
            ...
            {admin ? <><Link href="/admin/courses" className="flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-bold text-slate-600 hover:bg-slate-50"><BookOpen className="h-5 w-5" />Courses</Link>{adminPlaceholders.map(([Icon, label]) => <span key={label} title="Coming later" className="flex cursor-not-allowed items-center justify-between rounded-xl px-3 py-3 text-sm font-bold text-slate-400"><span className="flex items-center gap-3"><Icon className="h-5 w-5" />{label}</span><small className="text-[10px] uppercase">Soon</small></span>)}</> : (
              <>
